...
ActionInitialization::ActionInitialization(AnalysisManager* analysisMan, G4String outfile):
...
void ActionInitialization::BuildForMaster() const
{
	...
	// workers.
	SetUserAction(new RunAction(analysis, foutfile));
}
...
void ActionInitialization::Build() const
{
	...
	RunAction* run = new RunAction(analysis, foutfile);
	SetUserAction(run);
	
	EventAction* event = new EventAction(analysis);
	SetUserAction(event);

	SteppingAction* stepping = new SteppingAction(analysis, event);
